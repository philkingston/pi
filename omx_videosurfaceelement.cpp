...
OMX_VideoSurfaceElement::OMX_VideoSurfaceElement(QQuickItem *parent) :
		QQuickItem(parent), m_source(NULL), m_sgtexture(
				new OMX_SGTexture(0, QSize(0, 0))), m_textureSize(QSize(0, 0)), m_texData(
				NULL) {
...
QSGNode* OMX_VideoSurfaceElement::updatePaintNode(QSGNode* oldNode,
	...
	if (!oldNode) {
		...
		if (m_texData
				&& (GLuint) m_sgtexture->textureId() != m_texData->m_textureId)
	...
	vertices[0].set(bounds.x() + bounds.width(), bounds.y() + bounds.height(),
			1.0f, 1.0f);
	vertices[3].set(bounds.x(), bounds.y(), 0.0f, 0.0f);
	vertices[2].set(bounds.x() + bounds.width(), bounds.y(), 1.0f, 0.0f);
	return node;
}
